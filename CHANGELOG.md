...
### Changed
- Search now includes tool calls and tool output by default, including Bash commands, command output, and generated merge request links.
- Search result links open matching tool rows with tool output visible and highlighted.

### Added
- Regression coverage for global and per-session search over tool-only messages.
